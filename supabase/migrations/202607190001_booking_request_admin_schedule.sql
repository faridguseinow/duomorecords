...
begin
  ...
  if new_date is null or new_time is null then
    raise exception 'booking_slot_incomplete';
  end if;
  ...
  if exists (
    ...
    where b.id <> booking_id
      ...
      and b.booking_time = new_time
      and b.status in ('new', 'confirmed', 'in_progress')
  ) then
    raise exception 'slot_unavailable';
  end if;
  ...
  perform public.log_admin_activity(
    case when existing.booking_date is null then 'booking_scheduled' else 'booking_rescheduled' end,
    'booking',
    booking_id,
    'Booking ' || updated.booking_number || ' scheduled for ' || new_date || ' ' || new_time,
    jsonb_build_object('previous_date', existing.booking_date, 'previous_time', existing.booking_time, 'new_date', new_date, 'new_time', new_time, 'status', updated.status)
  );
